...
# # -------------------- 配置 --------------------
# SOURCE_DIR = "data/source_docs"               # 原始文档目录
# CHROMA_PERSIST_DIR = "./chroma_db"            # Chroma 持久化目录
# CHUNK_SIZE = 500                              # 切片大小（适当增大以适应长文档）
# CHUNK_OVERLAP = 100
...
# def ingest_documents(source_dir: str) -> List[Document]:
#     """
#     批量加载目录下所有支持格式的文档，返回 Document 列表。
#     """
#     facade = LoaderFacade()
#     all_docs = []
...
#                 print(f"⏭️ 跳过不支持格式: {file_path.name} ({e})")
#             except Exception as e:
#                 print(f"❌ 加载失败: {file_path.name} ({e})")
#     print(f"\n📄 总计加载 {len(all_docs)} 个文档片段")
#     return all_docs
# # data_ingestion.py 关键修改
...
#     # 按文件格式分组：Markdown 文档用 markdown 策略，其余用 recursive
#     md_docs = [d for d in docs if d.metadata.get("source", "").endswith(".md")]
#     other_docs = [d for d in docs if d not in md_docs]

#     chunks = []
#     if md_docs:
#         chunks.extend(factory.split(md_docs, strategy="markdown"))
#     if other_docs:
#         chunks.extend(factory.split(other_docs, strategy="recursive",
#                                     chunk_size=500, chunk_overlap=100))
#     print(f"🔪 切片完成: 总计 {len(chunks)} 个片段")
#     return chunks
...
#     print(f"✅ 已将 {len(chunks)} 个切片写入 Chroma ({CHROMA_PERSIST_DIR})")
...
#             print(f"  [{i}] {snippet}...")
#     print("\n✅ 验证完成，若上述结果包含相关信息则 PDF 内容已可检索。")

# # -------------------- 主流程 --------------------
# if __name__ == "__main__":
#     print("=" * 60)
#     print("  文档批量导入管道 (LoaderFacade)")
#     print("=" * 60)

#     # 1. 加载
#     docs = ingest_documents(SOURCE_DIR)
#     if not docs:
#         print("⚠️ 未加载到任何文档，请检查 data/source_docs/ 目录。")
#         sys.exit(0)
